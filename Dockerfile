...
# Run
CMD pulseaudio --start && printf "\n[spotify]\nusername = %s\npassword = %s\n" "$USER" "$PASS"  >> /etc/mopidy/mopidy.conf & mopidy --config /usr/share/mopidy/conf.d:/etc/mopidy/mopidy.conf local scan & mopidy --config /usr/share/mopidy/conf.d:/etc/mopidy/mopidy.conf local scan
